...
import { ArrowLeft, Columns3, LayoutTemplate, Pencil, Presentation, Rows3 } from 'lucide-react';
...
import {
  ...
  progressBadgeMeta,
  statusBadgeMeta,
} from '@/features/badges/constants/badge-catalog';
...
import { useTemplateActions } from '@/features/templates/hooks/useTemplates';
...
export function ProjectPage() {
  ...
  const areas: Array<{ title: string; text: string; onOpen?: () => void }> = [
    {
      title: 'Fases',
      text: 'Organize fases e edite os detalhes do planejamento.',
      onOpen: () => navigate(`/project/${id}/editor`),
    },
    { title: 'Timeline', text: 'A timeline sera construida em uma fase futura.' },
    {
      title: 'Tarefas',
      text: 'Acompanhe e organize as tarefas no quadro Kanban.',
      onOpen: () => navigate(`/project/${id}/kanban`),
    },
    {
      title: 'Estatisticas',
      text: 'As metricas detalhadas aparecerao conforme as fases forem estruturadas.',
  ...
  return (
    <section className="mx-auto flex w-full max-w-5xl flex-col gap-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-muted-foreground">
            {roadmap.description || 'Sem descricao.'}
          </p>
          <div className="mt-3 flex flex-wrap gap-1.5">
            ...
            />
            <Badge label={`v${roadmap.version}`} tooltip={`Versao ${roadmap.version}`} />
          </div>
        </div>
      ...
      <RoadmapFormDialog
        ...
        onSubmit={(values) =>
          actions.update.mutate(
            { id: roadmap.id, values },
            {
              onSuccess: () => {
                setEditing(false);
                toast.show('Projeto atualizado.');
              },
              onError: () => toast.show('Nao foi possivel atualizar o projeto.', 'error'),
            },
          )
        }
      />
      <SaveTemplateDialog
        roadmap={roadmap}
        open={savingTemplate}
        pending={templateActions.createFromRoadmap.isPending}
        onOpenChange={setSavingTemplate}
        onSubmit={(values) =>
          templateActions.createFromRoadmap.mutate(
            { roadmapId: roadmap.id, values },
            {
              onSuccess: () => {
                setSavingTemplate(false);
                toast.show('Template salvo na biblioteca.');
              },
              onError: () => toast.show('Não foi possível salvar o template.', 'error'),
            },
          )
        }
      />
