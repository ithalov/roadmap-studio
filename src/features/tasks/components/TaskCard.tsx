...
export function TaskCard({ task, tags, subtaskProgress, compact, dragAttributes, dragListeners, onOpen, onToggle, onDelete, onQuickUpdate }: TaskCardProps) {
  ...
  return <article className="flex items-start gap-3 rounded-lg border bg-card p-3 shadow-sm">
    ...
    <div className="min-w-0 flex-1 text-left">
      <div className="flex flex-wrap items-center gap-2"><input aria-label={`Titulo de ${task.title}`} className={task.completed ? 'min-w-32 bg-transparent font-medium line-through text-muted-foreground outline-none' : 'min-w-32 bg-transparent font-medium outline-none focus:ring-1 focus:ring-ring'} value={title} onChange={(event) => changeTitle(event.target.value)} onBlur={() => saveTitle(title)} /><Badge variant="status" size="sm" label={statusBadge.label} icon={statusBadge.icon} color={statusBadge.color} tooltip={status.description} /><Badge variant="priority" size="sm" label={priorityBadge.label} color={priorityBadge.color} tooltip={priority.description} /></div>
